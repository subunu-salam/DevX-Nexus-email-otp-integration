...
/* Gamified reward ladder — the "don't break the chain" pull the streak is
   built around. Escalating basket discounts, cashed in as a real personal
   offer coupon (lib/loyalty.js) so redemption flows through the same code
   the store already uses for every other coupon — no new payment path. */
const REWARD_TIERS = [
  { days: 3, pct: 5, label: '5% off your next order' },
  { days: 7, pct: 10, label: '10% off your next order' },
  { days: 14, pct: 15, label: '15% off your next order' },
  { days: 30, pct: 25, label: '25% off — your best streak yet' }
];
function rewardTierFor(days) { return REWARD_TIERS.find(t => t.days === Number(days)) || null; }
function unclaimedRewards(record) {
  const r = record || {}, cur = Number(r.current) || 0, claimed = Array.isArray(r.claimed) ? r.claimed : [];
  return REWARD_TIERS.filter(t => cur >= t.days && !claimed.includes(t.days));
}
...
function streakSummary(record,now=new Date()){
  ...
  return {current:Number(r.current)||0,best:Number(r.best)||0,lastDate:r.lastDate||null,weekly,monthly,claimed:r.claimed||[],nextReward:[3,7,14,30].find(x=>x>(Number(r.current)||0))||null,rewardTiers:REWARD_TIERS,unclaimedRewards:unclaimedRewards(r)};
}
module.exports={MIN_ORDERS,MIN_HISTORY_DAYS,completedOrders,historyDays,goalFor,dateKey,weekKey,monthKey,engageStreak,streakSummary,REWARD_TIERS,rewardTierFor,unclaimedRewards};
